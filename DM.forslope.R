...
fars. %<>%
  ...
  mutate ( pm.grp = ifelse(pm     %in% c('G130V', 'I154F'), 'miss', pm)) %>% 
  mutate ( pm.grp = ifelse(pm.grp %in% c('0', 'miss'), pm.grp, 'null' ) ) %>% 
  mutate ( pm.grp = ifelse(pm.grp == '0', 'homoz', pm.grp ) )
...
fars. %<>%
  left_join(bl) %>% 
  select(c(study, site, sjid, sev.o, aoo, gaa100, pm, pm.grp, adt, time., paramcd, bl, aval, amb, phase.n, dupline))
